...
func _process(_delta: float) -> void:
	...
	var desired_center := playfield.get_local_mouse_position() - drag_offset + Vector2(CELL_SIZE * 0.5, CELL_SIZE * 0.5)
...
func _check_position() -> void:
	var current_cell := _position_to_cell(draggable.position + Vector2(CELL_SIZE * 0.5, CELL_SIZE * 0.5))
...
func _position_to_cell(pos: Vector2) -> Vector2i:
	var x := clampi(int(round(pos.x / CELL_SIZE)), 0, GRID_COLS - 1)
	var y := clampi(int(round(pos.y / CELL_SIZE)), 0, GRID_ROWS - 1)
	return Vector2i(x, y)
